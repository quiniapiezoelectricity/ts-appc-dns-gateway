...
- **`appc-dns`** runs [dnsproxy](https://github.com/AdguardTeam/dnsproxy) as a DNS forwarder sidecar. Gateway forwards all queries that don't match a virtual IP to it on `127.0.0.1:5335`. The DNS sidecar resolves them against the configured upstreams (`DNS_UPSTREAM`) using its own bootstrap DNS — breaking the circular dependency that would otherwise occur if Gateway tried to resolve upstream hostnames through itself.
...
2. The connector node queries the gateway for DNS. Gateway checks its virtual IP table: if the domain has a mapping it returns the existing virtual IP; otherwise it assigns a new one (e.g. `10.250.0.1`). Tailscale's own domains are forwarded to the DNS sidecar directly and resolved to real IPs.
...
These two planes are coupled: the address families in `PRIVATEIP_CIDR` determine the gateway DNS `queryStrategy`, which constrains what outbound destination lookups can return. `DOMAIN_STRATEGY` can only further restrict egress within what the pool allows — it cannot expand it. The stack does not validate real host IPv6 reachability at startup, so family mismatches show up as request-time connection failures rather than a boot-time error.
...
| IPv6 only | IPv4 only | any | startup succeeds, but outbound requests fail — pool constrains DNS to IPv6 and the host has no IPv6 route |
| IPv6 only | dual-stack | any | IPv6 only |
| Dual-stack | dual-stack | `UseIPv4` *(default)* | IPv4 only |
| Dual-stack | dual-stack | `UseIP` | IPv4 and IPv6 |
| Dual-stack | IPv4 only | `UseIPv4` *(default)* | IPv4 only |
| Dual-stack | IPv4 only | `UseIP` | IPv4 destinations work; IPv6-only destinations fail per request |
